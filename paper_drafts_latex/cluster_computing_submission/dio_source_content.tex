...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
Large Language Model (LLM) serving systems maximize single-node throughput but struggle under heterogeneous GPU fleets and highly variable request costs. We present \textbf{DIO}, an \emph{engine-agnostic} control-plane orchestrator (OpenAI-compatible wrap over unmodified engines; model-family independent at the wire format, not a claim of universal multimodal/split-inference coverage). DIO learns per-worker ms/token slopes online with dual-timescale Normalized Least Mean Squares (NLMS)---$O(1)$ arithmetic per completion, not ``convergence in one request''---ranks workers by a joint cost (service estimate $+$ queueing $+$ tier $+$ VRAM $-$ cache), and applies Roofline-inspired admission decoupled from absolute $\hat{y}$. The stack is released as \texttt{dio-serve} (Python) with a Go reference plane; measured \texttt{pick()} overhead is $\sim$10--15\,$\mu$s. \textbf{Claim scope:} under fixed hardware, DIO targets \emph{relative cost ranking} for routing, not point-accurate latency prediction (dual-T4 MAPE remains high, $\sim$90--130\%). Empirically (separate regimes, never mixed): multi-seed CPU slope-skew \emph{simulation} steers $97.5\%$ of traffic to the fast worker; on \textbf{real dual Tesla T4 + stock vLLM} (Qwen2.5-3B) with one peer delay-throttled ($\times 2$, $n{=}10$), NLMS cuts p99 by $48.3\%\pm0.7\%$ vs.\ Round-Robin and beats $d{=}2$ RLS; on homogeneous dual-T4 ($n{=}10$), strategies are nearly tied---as expected when slopes match. We do not claim full production-parity against Orca/vLLM-distributed fleets or true multi-SKU GPUs in this draft.
\end{abstract}
...
\paragraph{Contributions (methods and evaluation scope---not numeric outcomes).}
\begin{enumerate}
    \item \textbf{Method: dual-timescale NLMS as a ranking signal.} Per-worker fast/slow slopes and bias with $O(1)$ updates per completion; absolute MAPE is reported and \emph{not} treated as a success metric. Admission is decoupled from absolute $\hat{y}$ (\texttt{rank\_only}/\texttt{empirical} modes).
    \item \textbf{Method: joint cost $+$ Roofline-style safety.} Scalar score combining service estimate, batch-amortized wait, tier, soft VRAM pressure, cache affinity, and hard VRAM blocks---without requiring engine patches.
    \item \textbf{System: non-invasive control plane.} OpenAI-compatible \texttt{dio-serve} library and Go reference plane over stock vLLM/compatible engines; scheduling overhead measured in microseconds.
    \item \textbf{Evaluation plan.} Multi-seed real dual-T4 (homogeneous $+$ delay-proxy skew), multi-seed slope-skew \emph{simulations} (clearly labeled), ablations of cost terms, NLMS vs.\ $d{=}2$ RLS with fixed cost, and open artifacts under \texttt{results\_workshop\_final/}. Numeric results live only in \S\ref{sec:evaluation}.
\end{enumerate}
We position novelty relative to multi-feature / engine-invasive predictors (e.g., NexusSched-style RLS) and static hetero allocators (M\'elange~\cite{melange}): DIO targets \emph{request-granularity} online ranking with explicit memory safety and deployability over unmodified engines.
...
The implementation clamps $s^{\text{fast}}_w \geq 0.1$\,ms/token to prevent degenerate zero-slope cold starts. Default cold-start priors ($s_0{=}2.0$\,ms/token, $b_0{=}150$\,ms) are deliberately mild for real engines; they still under-predict early dual-T4 samples and contribute to high MAPE until on the order of \textbf{15--20 completions per worker} accumulate (\S\ref{sec:mape})---we do \emph{not} claim ``convergence in $O(1)$ requests.'' The NLMS \emph{update} uses $O(1)$ floating-point operations per completion (computational complexity), versus $O(d^2)$ covariance updates for $d$-feature Recursive Least Squares (RLS) as in structurally informed predictors such as NexusSched~\cite{nexussched} (our comparison RLS uses $d{=}2$).
...
The manager exposes \texttt{POST /v1/chat/completions}, accepting standard OpenAI JSON (\texttt{messages}, \texttt{model}). Clients point any OpenAI SDK \texttt{base\_url} at the gateway (default port 8085) to obtain NLMS routing transparently. Optional headers \texttt{X-DIO-Tier} and \texttt{X-DIO-Session-Id} encode capability and affinity hints. The OpenAI-compatible surface is intentional: self-hosted engines (vLLM, SGLang, Ollama, TGI OpenAI mode) speak this wire format for many model families (Llama, Mistral, Qwen, \ldots). We call this \emph{engine-/wire-format agnostic}, not a claim of full multimodal, disaggregated PD, or split-inference coverage.
...
\begin{enumerate}
    \item \textbf{Regime~A --- Controlled multi-seed dual-T4 validation} (\S\ref{sec:e2e_real}): stock vLLM, Qwen2.5-3B-Instruct, $2$ identical Tesla T4s, HF tokenizer, $30$ req/seed, $\texttt{max\_tokens}{=}32$, $n{=}10$ seeds (workshop suite). Homogeneous claim (no invented skew; MAPE reported).
    ...
    \item \textbf{Regime~C --- Slope-skew heterogeneity} (\S\ref{sec:hetero}): multi-seed CPU simulation ($n{=}10$) plus real dual-T4 delay-throttle ($n{=}10$, Table~\ref{tab:real_hetero}; p99 $-48.3\%\pm0.7\%$ vs RR).
\end{enumerate}
...
\label{sec:baselines}
We compare DIO's \textbf{NLMS} predictor against, under an \emph{identical} joint cost function and the same dual-T4 / same vLLM engines where applicable:
\begin{itemize}
    \item \textbf{Round-Robin (RR)} --- common multi-instance default;
    \item \textbf{Least-Loaded (LL)} --- queue-depth heuristic;
    \item \textbf{RLS} ($d{=}2$, $\lambda{=}0.99$) --- covariance baseline approximating multi-feature online predictors without engine hooks.
\end{itemize}
All strategies sit in front of the \textbf{same stock vLLM} workers (same model, max tokens, GPU util). We do \textbf{not} claim a full production bake-off against Orca, DistServe, or multi-node vLLM-distributed with different batching/quantization---those require matched multi-node fleets beyond this draft's hardware. Primary metrics: \textbf{p50/p99 e2e} with mean$\pm$std and approximate 95\% CI half-widths across seeds; MAPE for prediction honesty; fraction to fast worker under skew. We \emph{separately} treat engine \textbf{TTFT} when available and never relabel e2e generation as TTFT.

\paragraph{What is simulation vs.\ real hardware.}
Tables and text mark \textbf{sim} (library slope-skew, calibrated mocks, Locust mock workers) vs.\ \textbf{real} (dual Tesla T4 + stock vLLM + Qwen2.5-3B). Delay-proxy Regime~C is real engines with controlled service-time inflation---not a second GPU SKU.
...
\label{sec:e2e_real}
This is the \textbf{primary real-hardware homogeneous} claim. Workshop final suite (\texttt{run\_workshop\_final\_suite.py}) on \textbf{dual Tesla T4} + stock \textbf{vLLM}, model \textbf{Qwen2.5-3B-Instruct}, HF tokenizer, $\texttt{max\_tokens}{=}32$, \textbf{$n{=}10$ seeds} $\times$ $30$ req/seed, strategies NLMS / RLS / RR / LL. Artifacts: \texttt{results\_workshop\_final/summary.json}. Setup differs sharply from Regime~B (Locust seconds): do not mix absolute scales. An earlier $n{=}3$ pilot (heuristic tokens, $\sim$900\,ms scale) is retained only as a pipeline note; Table~\ref{tab:e2e_dual_t4} is the multi-seed source of truth.
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Strategy} & \textbf{e2e p99 (ms)} & \textbf{MAPE \%} & \textbf{frac e0} & \textbf{vs RR} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{\textbf{Regime~A (primary).} Dual-T4 + vLLM + Qwen2.5-3B, HF tokenizer, $n{=}10$ seeds $\times$ 30 req, $\texttt{max\_tokens}{=}32$. Mean$\pm$std; 95\% CI half-width for NLMS p99 $\approx\pm19$\,ms. On \emph{identical} GPUs, p99 is nearly tied (NLMS does not invent gains). LL is sticky (all traffic to e0) with huge p99 variance. RLS mis-routes ($12\%$ to e0).}
\label{tab:e2e_dual_t4}
\end{table}

\paragraph{Reading homogeneous dual-GPU results.}
Both workers are the same SKU and model: large traffic skew is neither expected nor claimed. Mean p99 is within a few percent across NLMS/RR (NLMS is slightly worse than RR here: $-2.6\%\pm1.0\%$ ``improvement''). The useful homogeneous signals are (i)~NLMS keeps \textbf{tight seed-to-seed p99 std} ($31$\,ms) vs.\ LL ($430$\,ms), and (ii)~RLS ranking collapses even when workers are equal. Predictive routing's payoff appears when costs differ (Regime~C), not when they do not.

\paragraph{Absolute prediction error (MAPE) vs.\ relative ranking.}
\label{sec:mape}
We report MAPE between pre-feedback $\hat{y}$ and measured e2e \emph{explicitly}. On the workshop dual-T4 suite it is \textbf{high}: NLMS MAPE $123\pm6\%$ (tokenizer path). A controlled A/B (W2) finds HF tokenizer MAPE \textbf{worse} than the byte heuristic ($117.5\pm10.9\%$ vs.\ $89.7\pm4.9\%$, $n{=}3$)---so MAPE is \textbf{structural} (linear model $+$ cold start $+$ batching), not a counting artifact. \textbf{This does not invalidate the system story.} Routing uses $\arg\min_w S_w$. When costs differ, sim places $97.5\%$ of traffic on the fast worker and real throttle cuts p99 by $48.3\%\pm0.7\%$ ($n{=}10$; Table~\ref{tab:real_hetero}). We frame DIO as \textbf{relative cost ranking under fixed hardware}. Admission is decoupled from absolute $\hat{y}$ (\S\ref{sec:admission_decouple}).
...
Regime~A (Table~\ref{tab:e2e_dual_t4}) shows NLMS does not invent large homogeneous gains. The payoff is Regime~C with induced skew.

\paragraph{Real dual-T4 throttle validation ($n{=}10$, primary).}
\label{sec:real_hetero}
Workshop suite W4 wraps e1 with \texttt{latency\_delay\_proxy.py} ($\times 2$ wall-clock, real tokens), $n{=}10$ seeds $\times$ 30 req, HF tokenizer (Table~\ref{tab:real_hetero}). NLMS p99 is \textbf{$3427\pm38$\,ms} vs.\ RR \textbf{$6632\pm37$\,ms}: improvement \textbf{$48.3\%\pm0.7\%$} (95\% CI half-width $\approx\pm0.4$\,pp). RLS improves only $23.2\%\pm22.9\%$ with high variance and worse mean p99 ($5087\pm1505$\,ms). Mean fraction to e0 under NLMS is modest ($0.47\pm0.08$)---tail reduction, not a $97.5\%$-style sim split, is the real-GPU headline. Absolute MAPE remains high ($126\pm12\%$). Artifacts: \texttt{results\_workshop\_final/}. Together with Table~\ref{tab:t2_multiseed}, ranking is validated on \textbf{emulated CPU ($n{=}10$) and real GPU ($n{=}10$)}.
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Strategy} & \textbf{p99 (ms)} & \textbf{frac e0} & \textbf{vs RR} & \textbf{MAPE \%} \\
\midrule
NLMS & $\mathbf{3427 \pm 38}$ & $0.47 \pm 0.08$ & $\mathbf{48.3 \pm 0.7\%}$ & $126 \pm 12$ \\
RLS ($d{=}2$) & $5087 \pm 1505$ & $0.37 \pm 0.19$ & $23.2 \pm 22.9\%$ & $119 \pm 10$ \\
Round-Robin & $6632 \pm 37$ & $0.50 \pm 0.00$ & --- & $102 \pm 3$ \\
\bottomrule
\end{tabular}
\caption{\textbf{Regime~C (real GPU, primary).} Dual-T4 + vLLM + Qwen2.5-3B; e1 delay-proxy $\times 2$; $n{=}10$ seeds $\times$ 30 req; HF tokenizer. NLMS dominates RR on p99 with tight multi-seed variance; RLS is weaker and noisier.}
\label{tab:real_hetero}
\end{table}

\paragraph{Longer decode (multi-seed).}
\label{sec:long_decode}
Workshop W5 repeats homogeneous dual-T4 with $\texttt{max\_tokens}{=}128$, $n{=}3$ seeds $\times$ 20 req. NLMS p99 $13835\pm73$\,ms vs.\ RR $13821\pm88$\,ms (essentially tied; vs RR $-0.1\%\pm0.9\%$). Qualitative message matches Regime~A: without slope skew, predictive routing does not create a large p99 gap; absolute latency scales with decode length as expected.

\paragraph{Emulation calibration and convergence (measured, not asymptotic slogans).}
Calibrated mocks use published A100/T4 decode ratios ($\approx$2.5--3$\times$). We distinguish (i)~$O(1)$ \emph{arithmetic cost} of an NLMS update from (ii)~\emph{time-to-usable ranking}. Empirically, dual-T4 and sim runs need on the order of \textbf{15--20 completions per worker} before slopes leave cold-start priors and ranking becomes useful; under induced throttle, the fast slope moves first ($\mu_{\mathrm{fast}}{=}0.1$) while the slow slope trails ($\mu_{\mathrm{slow}}{=}0.01$). Figure~\ref{fig:req_dist} is a single-run routing snapshot; multi-seed aggregates appear in Tables~\ref{tab:t2_multiseed}--\ref{tab:real_hetero}.

\begin{figure}[!htb]
    \centering
    \includegraphics[width=\columnwidth]{fig_nlms_convergence_throttle.png}
    \caption{\textbf{NLMS dual-timescale response to an injected throttle (simulation).} At request $80$ the true ms/token slope jumps (thermal/interference stand-in). $s^{\mathrm{fast}}$ reacts first; $s^{\mathrm{slow}}$ lags; $|e_k|$ spikes then decays. This is a labeled synthetic illustration of adaptation dynamics---not dual-T4 wall-clock traces. Camera-ready should add the analogous plot from a real delay-proxy / power-cap run (\S\ref{sec:followups}).}
    \label{fig:nlms_conv}
\end{figure}

\paragraph{NLMS vs.\ RLS head-to-head (same cost function).}
\label{sec:rls_h2h}
We hold the joint cost fixed and swap only the predictor. \textbf{Simulation} (Table~\ref{tab:rls_h2h}): NLMS $100\%$ to fast vs.\ RLS $0.40\pm0.18$; pick() $\approx$9--10\,$\mu$s both. \textbf{Real dual-T4:} homogeneous cells in Table~\ref{tab:e2e_dual_t4}; throttle cells in Table~\ref{tab:real_hetero}. On real hardware, NLMS again beats RLS on the metric that matters (p99 under skew) without higher control-plane cost at $d{=}2$.
...
\bottomrule
\end{tabular}
\caption{\textbf{NLMS vs RLS vs RR vs LL (sim).} Regime~C slope-skew ($n{=}10$, 200 req/seed). Real dual-T4 RLS: Tables~\ref{tab:e2e_dual_t4}--\ref{tab:real_hetero}.}
\label{tab:rls_h2h}
\end{table}

\paragraph{Coefficient $\pm50\%$ on dual-T4.}
Workshop W6 scales tier/cache coefficients $\pm50\%$ on live homogeneous dual-T4. Observed p99 deltas vs.\ a single baseline run reach $\sim$50\%, but all non-baseline variants move together---consistent with \textbf{run-order / cold-start noise} on sequential restarts rather than stable coefficient sensitivity (tier/cache are largely inactive when workers match in tier and cache state). We therefore \textbf{do not} claim dual-T4 coefficient flatness from this sweep; the L4 grid-search claim stands, and dual-T4 coefficient isolation needs multi-seed interleaving (future).
...
As shown in Figure~\ref{fig:resilience}, DIO remains stable during convergence (T1), heterogeneity (T2), and cold starts (T3) in the L4 dual-worker microbench suite. Locust reports zero hard failures for the completed T2/T3 cloud CSVs under moderate load in that specific setup (dual mock/real workers on the L4 node, synthetic probes---not a 12{,}000-request multi-SKU fleet claim). In the \textbf{Cold Start (T3)} scenario, a newly registered worker starts from the default slope/intercept and begins receiving meaningful traffic after on the order of 15--20 telemetry-backed updates per worker (seconds in our dual-worker setup), which matters for elastic GPU scaling.
...
\label{sec:threats}
\begin{enumerate}
\item \textbf{No true mixed-SKU real fleet.} Real-GPU slope-skew uses a delay proxy on identical Tesla T4s (controlled service-time skew), not a physical T4+L4/A100 pair. Top external-validity gap for EuroSys/MLSys-tier claims.
\item \textbf{Baseline coverage.} We compare NLMS vs.\ RR / LL / $d{=}2$ RLS on the same vLLM backends. We do \emph{not} yet report head-to-head multi-node Orca, DistServe, or stock vLLM-distributed with independently tuned continuous batching---fairness would require matched fleets and configs.
\item \textbf{Absolute MAPE high; admission must not trust $\hat{y}$.} Workshop dual-T4 MAPE $\sim$90--130\%; HF tokenizer does not reduce it. Ranking-only / empirical admission modes apply (\S\ref{sec:admission_decouple}).
\item \textbf{Homogeneous p99 is not a NLMS win.} Under $n{=}10$ identical T4s, NLMS p99 is within $\sim$3\% of RR (slightly worse). Claims focus on skewed costs (Table~\ref{tab:real_hetero}) and ranking vs.\ RLS.
\item \textbf{Mean frac-to-fast under real throttle is modest} ($0.47\pm0.08$) even when p99 improves $\sim$48\%---tail behavior, not sim-style $97.5\%$ splits.
\item \textbf{Workload / scale.} Primary multi-seed real cells use short chat probes ($\texttt{max\_tokens}{=}32$ or $128$); concurrent closed-loop ShareGPT multi-seed on dual-T4 is incomplete. Control-plane scaling claims use mock workers for T7 (overhead $\mu$s is real; absolute e2e is synthetic).
\item \textbf{Out of scope.} Network-partitioned / pipeline-parallel models, multimodal split-inference, and privacy-preserving routing; for those settings split-inference or engine-invasive designs may remain preferable.
\item \textbf{Coefficient $\pm50\%$ on dual-T4} was inconclusive (run-order dominated); L4 grid-search is the coefficient evidence.
\item \textbf{Delay proxy $\neq$ SM thermal throttling.} Multiplies observed e2e, not GPU clocks. A true power-cap throttle experiment remains future work.
\item TTFT vs.\ e2e are reported separately after earlier metric confusion.
\end{enumerate}

\paragraph{Required follow-ups before a top-systems submission (checklist).}
\label{sec:followups}
(1)~At least one physical multi-SKU run (e.g., T4+L4). (2)~Matched baseline against multi-instance vLLM / Ray Serve under identical models. (3)~High-resolution convergence plots: $s_w$ and $|e_k|$ vs.\ request index with an injected throttle mid-trace. (4)~Concurrent (not only sequential) dual-T4 load. (5)~VRAM-growth traces with OOM comparison. (6)~Scale-out 2$\rightarrow$6 workers cold-start latency. Workshop-track claims can rely on the dual-T4 multi-seed + labeled sim evidence already reported.
...
We presented DIO, an engine-agnostic control plane for heterogeneous LLM inference: dual-timescale NLMS ranking, joint cost routing, admission decoupled from absolute $\hat{y}$, and \texttt{dio-serve}. Contributions are methods and evaluation scope; numbers live in \S\ref{sec:evaluation}. The defensible claim is \textbf{relative cost ranking under fixed hardware}, not low-MAPE prediction. Real dual-T4 multi-seed evidence (homogeneous $+$ delay-proxy skew, $n{=}10$) plus labeled simulations support workshop-track submission; top-systems venues still want multi-SKU hardware and broader production baselines (\S\ref{sec:followups}).

Future work: physical multi-SKU fleets; matched Orca/vLLM-distributed baselines; real-throttle convergence traces; concurrent ShareGPT multi-seed; 2$\rightarrow$6 scale-out cold start.
