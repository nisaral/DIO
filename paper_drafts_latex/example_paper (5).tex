...
Calibrated mocks use published A100/T4 decode ratios ($\approx$2.5--3$\times$). Convergence typically needs on the order of 15--20 completions per worker before slopes stabilize.
...
\label{sec:threats}
\begin{enumerate}
\item \textbf{No true mixed-SKU real fleet.} Real-GPU slope-skew uses a delay proxy on identical Tesla T4s (controlled service-time skew), not a physical T4+L4/A100 pair.
\item \textbf{Absolute MAPE high; admission must not trust $\hat{y}$.} Workshop dual-T4 MAPE $\sim$90--130\%; HF tokenizer does not reduce it. Ranking-only admission modes apply (\S\ref{sec:admission_decouple}).
...
\item \textbf{Longer decode} multi-seed is small ($n{=}3$, $\texttt{max\_tokens}{=}128$); Regime~B Locust remains single-seed.
\item \textbf{Coefficient $\pm50\%$ on dual-T4} was inconclusive (cold-start/run-order dominated); L4 grid-search is the coefficient evidence.
\item \textbf{Delay proxy $\neq$ SM throttling.} Multiplies observed e2e, not GPU clocks.
\item TTFT vs.\ e2e are reported separately.
\end{enumerate}
...
We presented DIO, an engine-agnostic control plane for heterogeneous LLM inference with dual-timescale NLMS, joint cost routing, admission decoupled from absolute $\hat{y}$, and \texttt{dio-serve}. The honest claim is \textbf{relative cost ranking under fixed hardware}, not low-MAPE prediction (workshop dual-T4 MAPE $\sim$90--130\%; tokenizer does not fix it). When costs differ, sim sends $97.5\%$ of traffic to the fast worker; on real dual-T4 vLLM with delay-proxy skew ($n{=}10$), NLMS cuts p99 by $48.3\%\pm0.7\%$ vs.\ RR and beats $d{=}2$ RLS. On homogeneous dual-T4 ($n{=}10$), p99 is nearly tied across strategies---as expected. Longer decode ($128$ tokens, $n{=}3$) remains qualitatively tied without skew.

Future work: true mixed-SKU fleets; multi-seed interleaved coefficient sweeps; larger $n$ at long decode; concurrent (not only sequential) load.
