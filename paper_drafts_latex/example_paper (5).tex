...
Our evaluation has four main limitations. First, most heterogeneity results use \emph{calibrated emulation} on single-GPU nodes; we disclose this explicitly and provide \texttt{emulation\_validation.json} plus optional real 2-GPU spot checks (\texttt{run\_t2\_validation.sh}) to bound emulation error. Second, each worker runs a single model; multi-model colocation may change VRAM dynamics. Third, production traces are replayed rather than live, preserving token distributions but not client retry feedback loops. Fourth, VRAM telemetry from NVML may lag sudden allocation spikes by one polling interval.
