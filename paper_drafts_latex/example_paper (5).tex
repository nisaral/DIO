...
\label{sec:mape}
We report mean absolute percentage error (MAPE) between NLMS's pre-feedback $\hat{y}$ and measured e2e latency \emph{explicitly}, rather than claiming accurate point prediction. On the dual-T4 suite, MAPE is \textbf{high}: smoke samples $\approx$67\%; multi-seed NLMS matrix seeds $\approx$76--82\%; dual-timescale vs.\ single-µ ablations $\approx$81.6\% vs.\ $\approx$85.9\% MAPE. Individual early samples illustrate the bias---e.g., $\hat{y}{=}193$\,ms vs.\ actual $856$\,ms, or $\hat{y}{=}55$\,ms vs.\ $283$\,ms---driven by (i)~conservative cold-start priors that climb slowly at $n{=}30$, (ii)~token features that approximate $\lfloor|\mathrm{prompt}|/4\rfloor{+}\texttt{max\_tokens}$, and (iii)~fixed overhead and batch effects that a scalar $sN{+}b$ model only partially captures. \textbf{This does not invalidate the system story.} Routing uses $\arg\min_w S_w$, so only the \emph{order} of costs matters. Regime~C shows that when costs truly differ, NLMS places $97.5\%$ of requests on the fast worker in simulation and $68.0\%\pm7.3\%$ on the unthrottled T4 under real vLLM ($n{=}5$)---evidence that relative ranking remains useful even while absolute MAPE stays large. We therefore frame DIO as \textbf{relative cost ranking under fixed hardware}, not as a low-MAPE latency oracle. Admission thresholds that depend on absolute $\hat{y}$ should be calibrated from measured capacity, not from cold-start priors alone.
...
\label{sec:threats}
(1)~\textbf{Absolute MAPE is high} on short dual-T4 runs ($\sim$66--95\%); claims are about relative ranking and tail stability, not low-error latency forecasting. (2)~\textbf{Statistical power.} Regime~A dual-T4 uses $n{=}3$ seeds; throttled real-GPU hetero uses $n{=}5$ (Table~\ref{tab:real_hetero}). (3)~\textbf{Homogeneous vs.\ heterogeneous.} Unthrottled dual-T4 SKUs match; slope-skew is Regime~C (sim $n{=}10$ $+$ real delay-proxy $n{=}5$). True mixed T4+L4/A100 remains future work. (4)~Regime~A/C request mix is synthetic chat with $\texttt{max\_tokens}{=}32$, not ShareGPT on dual-T4. (5)~Regime~B Locust seconds are single-seed and not commensurate with Regime~A ms. (6)~TTFT vs.\ e2e are separated after earlier metric confusion. (7)~Delay-proxy hetero multiplies observed e2e, not GPU SM clocks; it validates ranking under unequal service times on real engines, not a second physical SKU.
...
Future work: true mixed GPU SKUs (T4+L4/A100), longer multi-turn traces on dual-T4, TTFT-only goodput curves, and better absolute predictors (or admission calibration) when point accuracy is required.
