...
\begin{enumerate}
    ...
    \item \textbf{Regime~C --- Slope-skew heterogeneity} (\S\ref{sec:hetero}): multi-seed CPU simulation ($n{=}10$) as the primary ranking claim, plus a real dual-GPU path that throttles one peer via a latency delay proxy ($n{\ge}5$ seeds recommended).
\end{enumerate}
...
On Regime~A dual-T4 (identical SKUs, no throttle), NLMS does not invent large load skew (fraction to \texttt{e0} $\approx 0.43$ vs.\ RR $0.50$; p99 improvement $\approx 7.8\%\pm1.6\%$ on the unthrottled G3 subset)---expected when $s_w$ match. For a \textbf{real-engine} slope-skew check, the grand suite can wrap the second vLLM backend with \texttt{latency\_delay\_proxy.py} ($\times 2$ wall-clock, real tokens) and run G3 with $n{=}5$ seeds:
\begin{verbatim}
python scripts/run_gpu_cluster_validation.py \
  --engine-mode vllm --model Qwen/Qwen2.5-3B-Instruct \
  --gpus 0,1 --skip-g2 --skip-g4 --skip-g5 \
  --hetero-slow-mult 2.0 --hetero-seeds 5 \
  --requests-per-seed 30 --out results_gpu_cluster_hetero
\end{verbatim}
We treat that multi-seed throttled dual-T4 matrix as complementary validation (``emulated CPU $n{=}10$ $+$ real GPU $n{=}5$'') and will not substitute simulated frac\_fast for unfinished hardware cells. Artifacts: \texttt{results\_gpu\_cluster\_kaggle/} (homogeneous) and \texttt{results\_gpu\_cluster\_hetero/} (throttled, when run).
...
We presented DIO, an engine-agnostic control plane for heterogeneous LLM inference with dual-timescale NLMS, joint cost routing, admission control, and a pip-installable OpenAI-compatible wrap (\texttt{dio-serve}). The honest claim is \textbf{relative cost ranking under fixed hardware}, not low-MAPE point prediction: dual-T4 MAPE is $\sim$66--95\%, yet slope-skew multi-seed runs still send $97.5\%$ of traffic to the fast worker and cut p99 by $38.3\%\pm2.0\%$. On homogeneous dual Tesla T4 + stock vLLM (Qwen2.5-3B, $n{=}3$), NLMS achieves $898\pm9$\,ms mean p99 e2e versus $999\pm135$\,ms for Round-Robin at full success, with markedly lower variance. Controlled multi-seed validation, legacy Locust pilots, and slope-skew simulations are reported as separate regimes. We corrected TTFT/e2e confusion and documented deployment via unmodified engines.

Future work: finish multi-seed throttled dual-T4 hetero ($n{=}5$), true mixed GPU SKUs, longer multi-turn traces on dual-T4, TTFT-only goodput curves, and better absolute predictors (or admission calibration) when point accuracy is required.
