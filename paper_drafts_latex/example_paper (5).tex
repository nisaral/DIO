...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
\textbf{Large Language Model (LLM) serving systems typically maximize single-node throughput but struggle to maintain service-level objectives (SLOs) in heterogeneous, cluster-scale environments. Existing orchestrators often rely on engine-centric optimizations that lack cluster-wide visibility or distributed frameworks that depend on coarse-grained metrics, leading to head-of-line blocking and memory failures when hardware capabilities vary. We introduce DIO, a distributed inference orchestrator that decouples predictive scheduling from execution. By employing a dual-timescale Normalized Least Mean Squares (NLMS) predictor, DIO learns per-worker performance characteristics online without static profiling. This learned model drives a Roofline-inspired admission control system that explicitly manages VRAM constraints, preventing out-of-memory crashes while optimizing tail latency across diverse GPU fleets. On production traces replayed across A100 and L4 hardware, DIO reduces p99 Time-To-First-Token (TTFT) by up to 63\% on our ShareGPT workload with emulated heterogeneity, eliminates OOM failures across all 12 stress tests, and adds only 14\,$\mu$s of scheduling overhead per request---all without modifying the underlying inference engine.}
\end{abstract}
...
Figure~\ref{fig:line_comparison} (Top Row) reports p99 TTFT. On \textbf{ShareGPT}, DIO achieves 2.38\,s p99 TTFT vs.\ 5.40\,s for RR by predicting generation length and mitigating head-of-line blocking. On \textbf{arXiv}, RR's memory blindness causes severe VRAM thrashing, pushing p99 TTFT to \textbf{82.5\,s}. DIO's VRAM-aware routing identifies this pressure and steers requests to capable workers, capping p99 TTFT at 13.5\,s---a \textbf{6${\times}$ reduction}.

\subsubsection{SLO Attainment and Goodput}
Raw throughput metrics can be misleading if the generated tokens arrive too late to be useful. We define goodput as the throughput of requests that meet the $\le$2\,s TTFT target.

As shown in Figure~\ref{fig:line_comparison} (Bottom Row), DIO improves SLO attainment significantly. On \textbf{ShareGPT}, we achieve \textbf{96.2\%} (vs.\ 78.5\% for RR). On \textbf{arXiv}, RR drops to 34\% due to thrashing, while DIO maintains \textbf{71\%}. On \textbf{Azure Code}, unpredictable decoding leads RR to only 12\% attainment, while DIO's adaptive feedback boosts this to \textbf{88\%}.
...
We presented DIO, an engine-agnostic orchestrator for heterogeneous LLM inference clusters. DIO combines three mechanisms: a dual-timescale NLMS predictor that learns per-worker ms/token slopes online in $O(1)$ time, Roofline-inspired admission control that proactively prevents OOM failures, and a tier-aware cost function that enforces capability constraints in multi-model deployments. Across 12 experiments on A100 and L4 hardware with production traces, DIO reduced p99 TTFT by up to 63\% on our ShareGPT workload with emulated heterogeneity, eliminated OOM failures in all 12 stress tests (including VRAM-constrained arXiv summarization), and added only 14\,$\mu$s of scheduling overhead per request---all without modifying the underlying vLLM or SGLang engines.
