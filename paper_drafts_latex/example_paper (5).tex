...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
Large Language Model (LLM) serving systems maximize single-node throughput but struggle under heterogeneous GPU fleets and highly variable request costs. We present \textbf{DIO}, a model-agnostic control-plane orchestrator that decouples predictive routing from engine execution. DIO learns per-worker ms/token slopes online with a dual-timescale Normalized Least Mean Squares (NLMS) filter ($O(1)$ updates), combines predicted service time with queueing, tier, and VRAM terms in a cost-based router, and applies Roofline-inspired hard admission when all workers are predicted unsafe. The control plane is non-invasive (OpenAI-compatible gateway over unmodified vLLM), adds sub-millisecond scheduling overhead, and is released as \texttt{dio-serve}. On a \textbf{real dual-T4} cluster serving Qwen2.5-3B-Instruct via stock vLLM ($n{=}3$ seeds), NLMS improves mean p99 end-to-end latency to \textbf{$898\pm9$\,ms} versus Round-Robin \textbf{$999\pm135$\,ms} ($\approx$10\% lower mean p99, with far lower seed-to-seed variance) at $100\%$ success. In controlled multi-seed slope-skew simulations, NLMS steers $97.5\%$ of traffic to the fast worker and cuts p99 by $38.3\%\pm2.0\%$. We report homogeneous dual-GPU, multi-seed microbenchmarks, and package-level validation without fabricating hardware outcomes.
\end{abstract}
...
% ------------------------------------------------------------------
\subsection{End-to-End Performance on Real Dual-GPU Hardware}
\label{sec:e2e_real}
We ran the packaged \texttt{dio-serve} grand suite on a \textbf{dual Tesla T4} node (Kaggle) with stock \textbf{vLLM} OpenAI servers, model \textbf{Qwen2.5-3B-Instruct}, $\texttt{max\_tokens}{=}32$, and \textbf{$n{=}3$ independent seeds} $\times$ $30$ requests/seed ($90$ successful requests per strategy, $0$ failures). DIO sat in front of two identical backends (\texttt{:18000}, \texttt{:18001}) with no engine patches. Results are archived in \texttt{results\_gpu\_cluster\_kaggle/summary.json}.
...
\bottomrule
\end{tabular}
\caption{Dual-T4 + vLLM + Qwen2.5-3B multi-seed matrix (mean$\pm$std over $n{=}3$ seeds). Latencies are end-to-end through DIO, not TTFT. NLMS does not invent large gains on \emph{identical} GPUs; its main benefit here is \textbf{stable tails} (std $9$\,ms vs.\ $135$\,ms for RR).}
...
\begin{figure*}[t]
    ...
    \caption{\textbf{Legacy single-seed Locust pilot (A100-class / 4 logical workers, Llama-3.2-3B).} Retained for pipeline reproducibility only; multi-seed dual-T4 results in Table~\ref{tab:e2e_dual_t4} are the primary real-hardware claim.}
    \label{fig:line_comparison}
\end{figure*}

\paragraph{Legacy Locust pilot (secondary).}
Table~\ref{tab:e2e_summary} and Figure~\ref{fig:line_comparison} retain an earlier single-seed ShareGPT/arXiv/Azure pilot for pipeline transparency. Those absolute seconds (tens of seconds) reflect longer decode budgets under Locust concurrency and should not be mixed with the dual-T4 millisecond-scale runs above.
...
\bottomrule
\end{tabular}
\caption{Legacy single-seed Locust pilot (not multi-seed). Prefer Table~\ref{tab:e2e_dual_t4} for real dual-GPU claims.}
...
\paragraph{Homogeneous dual-T4 vs.\ slope-skew simulation.}
On the dual-T4 vLLM run (\S\ref{sec:e2e_real}), both backends are the same SKU and model, so NLMS does not invent large load skew (fraction to \texttt{e0} $\approx 0.43$ vs.\ RR $0.50$; p99 improvement $\approx 7.8\%\pm1.6\%$ on the G3 subset). That is expected: predictive routing shines when $s_w$ differs. Table~\ref{tab:t2_multiseed} isolates the \emph{slope-skew} regime in multi-seed simulation. Together, the two tables show (i)~stable tails on homogeneous dual-GPU production path and (ii)~strong rebalancing when workers actually differ.

\paragraph{Emulation calibration and package validation.}
Calibrated mocks use published A100/T4 decode ratios ($\approx$2.5--3$\times$). The packaged library was also smoke-tested with real HF engines and live dual-T4 vLLM (this section); raw artifacts live under \texttt{results\_gpu\_cluster\_kaggle/}.
...
\label{sec:threats}
(1)~\textbf{Statistical power.} Dual-T4 Table~\ref{tab:e2e_dual_t4} uses $n{=}3$ seeds (workshop-acceptable minimum); more seeds and longer runs would tighten CIs, especially for RR. (2)~\textbf{Homogeneous vs.\ heterogeneous GPUs.} Dual-T4 workers match in type, so large routing skew is not expected; slope-skew gains are isolated in Table~\ref{tab:t2_multiseed}. A mixed T4+L4/A100 run remains future work. (3)~Request mix is synthetic chat probes with \texttt{max\_tokens}{=}32, not full ShareGPT replay on the dual-T4 node. (4)~Single model per worker. (5)~Legacy Locust absolute seconds (Table~\ref{tab:e2e_summary}) are single-seed and decode-budget-dependent. (6)~We separate TTFT from e2e reporting after earlier metric confusion.
...
We presented DIO, an engine-agnostic control plane for heterogeneous LLM inference with dual-timescale NLMS, joint cost routing, admission control, and a pip-installable OpenAI-compatible wrap (\texttt{dio-serve}). On dual Tesla T4 + stock vLLM (Qwen2.5-3B, $n{=}3$ seeds), NLMS achieves $898\pm9$\,ms mean p99 e2e versus $999\pm135$\,ms for Round-Robin at full success, with markedly lower variance. When worker slopes truly differ, multi-seed simulations show $38.3\%\pm2.0\%$ p99 reduction and $97.5\%$ traffic to the fast worker. We corrected TTFT/e2e metric confusion and documented deployment via unmodified engines.

Future work: mixed GPU types on one node, longer multi-turn traces on dual-T4, TTFT-only goodput curves, and conversation-level KV affinity beyond prefix-hash bonuses.
