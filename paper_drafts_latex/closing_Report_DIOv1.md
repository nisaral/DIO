...
**Datasets:**
1. **Synthetic:** Uniform length distribution (Low Variance)
2. **ShareGPT:** Real-world conversation traces (High Variance / Power Law)
...
| Metric | Synthetic Data | ShareGPT (Real Data) | Degradation |
|--------|----------------|----------------------|-------------|
| Total Requests | 4,739 | 4,314 | N/A |
| Error Rate | 0.00% | 0.00% |  PASS (Stable) |
| Median Latency (p50) | 840ms | 860ms | +2.3% (Negligible) |
| Tail Latency (p95) | 1,800ms | 2,000ms | +11.1% |
| Tail Latency (p99) | 2,100ms | 3,000ms | **+42.8% (CRITICAL)** |
...
1. Worker A receives a **Giant Prompt** (2s execution)
2. Worker A immediately receives a **Tiny Prompt** (50ms execution) because it is "next in line"
3. The Tiny Prompt **sits in the queue for 2 seconds** waiting for the Giant Prompt to finish
4. **Result:** A 50ms task takes 2,050ms — this drives the p99 metric up, even if the hardware is powerful
...
1. **Cost Estimation:** A Linear Regression model (`Cost = 2.1 × Tokens + 50`) will predict the duration of incoming requests

2. **Priority Queueing:** Instead of a FIFO queue, requests will be sorted by `Predicted Cost`

3. **Impact:** Short tasks will "jump the line," executing immediately on available workers. This mathematically minimizes the Average Waiting Time.
